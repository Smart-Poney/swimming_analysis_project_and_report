...
AICmultitab <- function(cloud) {
  ...
  get_random <- function(x) {
    ...
    re <- tryCatch(names(x$lme$modelStruct$reStruct),
    ...
    paste(re, collapse = " + ")
  } # obtenir les effets alétoires des GAMM dans leur formule (n'est pas décrit dans modelGAMM$gam)
 ...
 #nature modèle
  ...
  get_formula <- function(x) {
    if (is_gamm(x)) {
      gam_formula <- paste(deparse(formula(x$gam)),collapse = " ")
      rand_formula <- get_random(x)
      return(paste0(gam_formula," | random = ",rand_formula))
    }
