...
dataexport <- data %>% dplyr::select(id, y_norm, X_xnorm, points,trace, bief, segment, traitement, taille_mm, poids_g, zone,sequence)

save(dataexport, file = "OUTPUT/dataexport.RData")

# Étape 5 : prendre les max ####

# SANS valeurs absolues

datamax <- dataexport %>% group_by(id, trace) %>%
...
datamin <- dataexport %>% group_by(id, trace) %>%
...
abs <- ggplot(data = datamaxabs, aes(x = X_norm, y = Y_max, group = interaction(id, segment),
                           color = traitement)) +
  labs(subtitle = "Stationnaires", x = "Position normalisée (X)",
...
save(datamaxabs, file = "OUTPUT/dataAMAV02.RData")
