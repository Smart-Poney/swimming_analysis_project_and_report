#
# 23/07/26 - 03.animation_wallpaper_personal.R
#
...
# latest modification : 23/07/2026
#
...
# background image ----
background_image <- "wallpaper/nepal.jpeg"
bg_img <- jpeg::readJPEG(background_image)
#bg_img <- png::readPNG(background_image)

figure_width <- 14.14 # Export et paramètres de GIF 
figure_height <- 10
dpi_export <- 300
...
alpha_max <- 0.60      # silhouette actuelle
...
img_ratio <- 6614 / 4677 #propriétés image
current_height <- ymax - ymin
target_width   <- current_height * img_ratio
current_width <- xmax - xmin
if(target_width > current_width){
  marge <- (target_width - current_width)/2
  xmin <- xmin - marge
  xmax <- xmax + marge
}

cat("largeur =", xmax-xmin, "\n")
cat("hauteur =", ymax-ymin, "\n")
cat("ratio =", (xmax-xmin)/(ymax-ymin), "\n")
